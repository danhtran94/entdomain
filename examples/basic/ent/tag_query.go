...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withUsers; query != nil {
		if err := _q.loadUsers(ctx, query, nodes,
			func(n *Tag) { n.Edges.Users = []*User{} },
			func(n *Tag, e *User) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TagQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.UsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(tag.UsersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.UsersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.UsersPrimaryKey[1]))
