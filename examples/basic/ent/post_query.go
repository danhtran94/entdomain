...
func (_q *PostQuery) Clone() *PostQuery {
	...
	return &PostQuery{
		...
		withPinners: _q.withPinners.Clone(),
...
// WithPinners tells the query-builder to eager-load the nodes that are connected to
// the "pinners" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *PostQuery) WithPinners(opts ...func(*UserQuery)) *PostQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withPinners = query
	return _q
}
...
func (_q *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := _q.withPinners; query != nil {
		if err := _q.loadPinners(ctx, query, nodes,
			func(n *Post) { n.Edges.Pinners = []*User{} },
			func(n *Post, e *User) { n.Edges.Pinners = append(n.Edges.Pinners, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PostQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Post, init func(*Post), assign func(*Post, *User)) error {
...
func (_q *PostQuery) loadPinners(ctx context.Context, query *UserQuery, nodes []*Post, init func(*Post), assign func(*Post, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(post.PinnersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_pinned_post
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_pinned_post" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_pinned_post" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
